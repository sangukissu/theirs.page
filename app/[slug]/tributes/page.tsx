...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!context) return buildNoIndexMetadata("Tributes")
  if (context.redirectedToSlug) {
    permanentRedirect(`/${context.redirectedToSlug}/tributes`)
  }
  ...
  return buildMemorialMetadata({
    ...
    slug,
    isSubpage: true,
    subpageTitle: "Tributes & Condolences",
    subpagePath: "tributes",
  })
}
...
export default async function TributesPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="pt-16 sm:pt-20 pb-10">
      <PagedTributes slug={slug} fullName={context.identity.fullName} memorialId={context.identity.id} isDemo={context.identity.isDemo} initial={page} />
    </div>
  )
}
