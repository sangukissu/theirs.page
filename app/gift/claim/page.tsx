...
export default async function GiftClaimPage({ searchParams }: ClaimPageProps) {
  ...
  if (!token || token.length < 32) {
    return (
      <div className="min-h-screen bg-white text-[#666] flex flex-col selection:bg-primary/10 selection:text-primary">
        <TheirsNav />

        <main className="flex-1 flex items-center justify-center p-6">
          <div className="w-full max-w-md rounded-2xl border border-black/[0.08] bg-[#f7f7f8] p-8 text-center flex flex-col items-center gap-4">
            ...
            <h1 className="text-2xl font-medium text-[#181925] tracking-tight">
            ...
            <Link
              href="/"
              className="mt-2 inline-flex items-center justify-center h-10 px-5 rounded-full border border-black/[0.12] bg-white text-xs font-medium text-[#181925] hover:bg-[#fafafb] transition-colors"
            >
              Return to homepage
            </Link>
          </div>
      ...
      <div className="min-h-screen bg-white text-[#666] flex flex-col selection:bg-primary/10 selection:text-primary">
        ...
        <main className="flex-1 flex items-center justify-center p-6">
          <div className="w-full max-w-md rounded-2xl border border-black/[0.08] bg-[#f7f7f8] p-8 text-center flex flex-col items-center gap-4">
            ...
            {redeemedMemorial && (
              <div className="w-full rounded-xl border border-black/[0.08] bg-white p-4 text-left flex flex-col gap-1 my-2">
                <p className="font-mono text-xs uppercase tracking-wider text-[#777]">
                  Activated Memorial:
                </p>
                <p className="font-medium text-sm text-[#181925]">{redeemedMemorial.full_name}</p>
                <p className="font-mono text-xs text-primary">theirs.page/{redeemedMemorial.slug}</p>
              </div>
            )}
    ...
    <div className="min-h-screen bg-white text-[#666] flex flex-col selection:bg-primary/10 selection:text-primary">
      ...
      <main className="flex-1 py-12 sm:py-16 px-4 sm:px-6 flex flex-col items-center">
        <div className="w-full max-w-xl text-center mb-8">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-black/[0.08] bg-[#f7f7f8] text-xs font-mono uppercase tracking-wider text-primary font-medium mb-3">
            <Gift className="size-3.5" />
            <span>Memorial Gift Invitation</span>
          </div>

          <h1 className="text-3xl sm:text-4xl font-medium tracking-tight text-[#181925] leading-tight">
            You have received a memorial gift
          </h1>
          <p className="mt-3 text-sm leading-6 text-[#666] max-w-md mx-auto">
