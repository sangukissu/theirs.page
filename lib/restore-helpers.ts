...
export function validateMemorialRestoreKey(key: string, memorialId: string, userId: string): boolean {
  if (typeof key !== "string" || key.length === 0 || key.length > 1024) {
    return false
  }
  ...
  const parts = key.split("/")
  if (parts.length >= 4 && parts[0] === "temp" && parts[1] === "restorations" && parts[2] === userId) {
    return true
  }
  if (parts.length >= 4 && parts[0] === "memorials" && parts[1] === memorialId && parts[2] === "restorations") {
    return true
  }
  return false
}
...
export async function preserveMemorialOriginalForComparison(
  sourceKey: string,
  memorialId: string,
  ...
  const destinationKey = `memorials/${memorialId}/restorations/${restorationId}/original.${extension}`
