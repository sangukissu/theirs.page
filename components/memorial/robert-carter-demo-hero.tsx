...
import { HeroGradientAura, type AuraMode, type AuraColor } from "./hero-gradient-aura"
import type { MemorialThemeId } from "@/lib/memorial/themes"

interface RobertCarterDemoHeroProps {
  fullName: string
...
export function RobertCarterDemoHero({
  fullName,
  preferredName,
  birthYear = null,
  deathYear = null,
  ...
  return (
    <div className="relative w-full overflow-hidden isolate">
      {/* Background Gradient Mask Layer (Dual Left & Right) */}
      <HeroGradientAura
        opacity={auraOpacity}
        mode={auraMode}
        color={auraColor}
      ...
      <section className="relative z-10 pt-24 sm:pt-32 pb-12 sm:pb-16 px-4 max-w-4xl mx-auto text-center flex flex-col items-center">
        {/* Museum-Grade Archival Portrait Frame with Theme Matting */}
        <ThemeHeroFrame themeId={themeId}>
          <div className="relative size-32 sm:size-40 rounded-2xl overflow-hidden bg-neutral-100/50 border border-[var(--theme-border)] shadow-sm">
            {portraitUrl ? (
              <>
                <img
                  src={portraitUrl}
        ...
        <div className="flex flex-col items-center gap-2 max-w-2xl">
          ...
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-[var(--theme-bg-surface-subtle)] border border-[var(--theme-border)] text-xs font-mono text-[var(--theme-text-muted)] mt-1 select-none max-w-full backdrop-blur-xs">
            <span>{yearsSpan}</span>
            {age && (
              <>
          ...
          <div className="mt-4 flex flex-col sm:flex-row items-center gap-3 select-none">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-1.5 whitespace-nowrap !rounded-full font-medium transition-all cursor-pointer border border-[var(--theme-border)] bg-[var(--theme-bg-surface)] text-[var(--theme-text-primary)] shadow-2xs hover:bg-[var(--theme-bg-surface-subtle)] active:scale-[0.98] h-10 px-5 text-sm"
            >
