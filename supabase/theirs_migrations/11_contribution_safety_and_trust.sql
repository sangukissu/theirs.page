...
alter table public.memories add column if not exists safety_decision text default 'safe' check (safety_decision in ('safe', 'review', 'blocked'));
alter table public.memories add column if not exists safety_details jsonb default '{}'::jsonb;
alter table public.memories add column if not exists contributor_role text default 'anonymous' check (contributor_role in ('anonymous', 'invited', 'trusted', 'co_admin', 'owner'));
alter table public.memories add column if not exists is_quarantined boolean default false;
...
alter table public.memorials add column if not exists contribution_settings jsonb default '{"accept_contributions":true,"tributes":true,"memories":true,"photos":true,"voice":true,"videos":true,"moments":true}'::jsonb;

-- 4. Fast Query Indexes
create index if not exists idx_memories_receipt_token on public.memories(receipt_token);
